...
#[cfg(test)]
mod tests {
    use super::{Basic, BitDepth, InputKind, Interface, ScreenSize};
    ...
    fn examples_docstring() {
        ...
        assert!(
            matches!(basic.screen_size(), ScreenSize::Dimensions(size) if size.width == 480 && size.height == 270)
        );
        assert!((basic.gamma().unwrap() - 2.2).abs() < f32::EPSILON);
        assert!(basic.features().standard_rgb());
        assert!(basic.validate(true).is_valid());
